...
import os
import time
# import BasicTokenizer and RegexTokenizer
...
text = open("/mnt/atlas_local/brady/home/projects/karpathy_DL_tutorials/build_nanoGPT/small_GPT_hp/harry_potter_all_books.txt", "r", encoding="utf-8").read()

# create a directory for models, so we don't pollute the current directory
os.makedirs("models", exist_ok=True)

# if models/regex.model and models/basic.model do not exist, train them
if not os.path.exists("models/regex.model") or not os.path.exists("models/basic.model"):
    t0 = time.time()
    for TokenizerClass, name in zip([RegexTokenizer, BasicTokenizer], ["regex", "basic"]):
    ...
    # Initialize the tokenizers if necessary
    tokenizers = RegexTokenizer(), BasicTokenizer()
# train and test splits
# load the tokenizer (either regex or basic)
tokenizer = tokenizers[0] # RegexTokenizer
...
with open("harry_potter_text_nano_GPT_all_books.txt", "w") as file:
    generated_text = tokenizer.decode(m.generate(context, max_new_tokens=10000)[0].tolist())
